...
3. 输入 Token 登录
4. 连接你的第一个消息平台（微信、Telegram、Discord 等）
5. 如果启用了本地模型，已自动配置好——创建智能体时选择本地模型即可
...
1. 复制部署日志中显示的 **网关访问令牌（Token）**
2. 在浏览器打开 **http://\<Jetson IP\>:18789**
3. 输入 Token 登录
4. 连接你的第一个消息平台
5. 如果启用了本地模型，已自动配置好并使用 GPU 加速——创建智能体时选择本地模型即可
...
1. 复制部署日志中的 **网关访问令牌（Token）**
2. 在浏览器中打开 OpenClaw 管理界面，输入 Token 登录
